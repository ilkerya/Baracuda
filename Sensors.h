...
#define CHANNEL_ACCEL_GYRO_0 0  // 0x77 default address LSM9DS1 accoloremoter/Gyroscope/MAgnetometer 
#define CHANNEL_LIDAR_1 1
...
#endif
#ifdef LIDAR_EXISTS
void Sensor_LidarInit() {
  tcaselect(CHANNEL_LIDAR_1);
  myLidarLite.begin(0, true); // Set configuration to default and I2C to 400 kHz
  myLidarLite.configure(0);
}
void Sensor_LidarRead() {
  tcaselect(CHANNEL_LIDAR_1);
  Values.Lidar = myLidarLite.distance();
}
#endif
...
void SensorACccel_GyroInit(void) {
 ...
 // tcaselect(CHANNEL_ACCEL_GYRO_1);
  ...
  else Serial.println(F("IMU Init Success"));
  /*
    else
...
float roll_2 = 0.00, pitch_2 = 0.00,  yaw_2 = 0.00;
...
void RP_calculate(){
  ...
  roll_2 = atan2(y_Buff , z_Buff) * 57.3;
  pitch_2 = atan2((- x_Buff) , sqrt(y_Buff * y_Buff + z_Buff * z_Buff)) * 57.3;
}
...
void SensorAcccel_GyroRead() {
 ...
 // Magnetometer.x = m.magData.x;
  Magnetometer.y = lsm.magData.y;
//  Magnetometer.z = _magSensor.z;
...
/*
   x_Buff = float(Accelometer.x);
   y_Buff = float(Accelometer.y);
   z_Buff = float(Accelometer.z);

*/
  roll_2 = atan2(Accelometer.y , Accelometer.z)* 180 / PI; // * 57.3;
 // pitch_2 = atan2((- Accelometer.x) , sqrt(Accelometer.y * Accelometer.y + Accelometer.z * Accelometer.z)) * 57.3;
  pitch_2 = atan2(-1 * Accelometer.x ,  Accelometer.z ) * 57.3;
 //  RP_calculate();
  yaw_2 = atan2( Accelometer.x, Accelometer.y ) * 180 / PI;
   

  roll = atan(Accelometer.y / sqrt(pow(Accelometer.x, 2) + pow(Accelometer.z, 2))) * 180 / PI;
  pitch = atan(-1 * Accelometer.x / sqrt(pow(Accelometer.y, 2) + pow(Accelometer.z, 2))) * 180 / PI;
  yaw = atan( Accelometer.x / sqrt(pow(Accelometer.z, 2) + pow(Accelometer.y, 2))) * 180 / PI;
